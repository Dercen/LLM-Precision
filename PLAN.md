...
| opt-1.3b | 2.63 GB | ~3.3 GB | resident |
| opt-2.7b | 5.30 GB | ~5.6 GB measured estimate | **marginal — see below** |
...
**opt-2.7b is marginal, not comfortable.** Capacity is 7.62 − 0.8 = 6.82 GB and the rule demands 1.3 × 5.3 = 6.89 GB, so opt-2.7b falls just the wrong side of the line. The 30% allowance is sized for full-logits evaluation; with the chunked CE of §5b the real activation cost is a few hundred MB, and the realistic resident peak is ~5.6 GB — it almost certainly fits. Rather than tune the constant against an estimate, the tiering of opt-2.7b is left to a measured `peak_vram_gb` at M4, with `--eval-mode resident` available to force it. `tests/test_env.py::test_opt_2_7b_is_the_marginal_case` pins the current arithmetic so any change to the factor or the reserve is deliberate.
...
Streamed tier, host-RAM cost with §5c window batching at 32 windows:
...
| `c4` | `load_dataset("allenai/c4", data_files={"validation": "en/c4-validation.00000-of-00008.json.gz"}, split="validation")` | shard 0, 45,576 docs | none | `random.seed(0)`, 256 docs redrawn until >=2048 tokens, one random 2048 window each. No config name. GPTQ `get_c4`; `protocol_uncertain: true` |
| `c4_new` | same shard | validation | `" ".join(docs[:1100])`, first 256×2048 tokens | GPTQ `--new-eval` variant, opt-in |
| `ptb` | `load_dataset("ptb-text-only/ptb_text_only", revision="refs/convert/parquet")` | validation, 3,370 rows, column `sentence` | `"\n\n".join` | GPTQ `get_ptb`; `protocol_uncertain: true` |
...
| **M2** Data + RTN + schema + CI | c4, ptb, c4_new, ptb_new, calibration loop, `fakequant` with the reference grid and the explicit fp32 round-trip, `rtn`, JSON schema, token-hash and tokenizer-class tests, `test_matches_reference_quantizer` (CPU and GPU), chunked-CE equivalence test, smoke suite, CI | wikitext2 is the only hard gate: **RTN4 per-row 37.28 ± 1%**, RTN8 within 0.05 of fp. `--ce-chunk 256` and `2048` agree to 1e-4. Advisory: run both ptb and ptb_new (c4 and c4_new), record which lands within 1% of 38.99 (26.56), set that as the paper column. CI green | 1.5 days |
| **M3** GPTQ + the streaming primitive | `eval/streaming.py`, `sequential` with the kwargs-replaying Catcher, `gptq`, `--stream-blocks` eval mode, opt-350m and SmolLM2-135M runs, `test_parity`, `test_streaming` | **GPTQ4 per-row 31.12 ± 3%, GPTQ3 per-row 53.85 ± 5%** on opt-125m; ptb 45.17 and c4 29.22 advisory on the M2-chosen key; opt-350m advisory fp16 22.00, RTN4 25.94, GPTQ4 24.24, RTN3 64.57, GPTQ3 33.79; SmolLM2 GPTQ4 finite and below RTN4 (proves the Llama family map before any real Llama); **resident and streamed eval agree to 1e-4 on opt-350m**; a repeated eval is bit-identical (§5a); 3-seed spread recorded | 3 days |
| **M4** Runner + plots + AWQ-lite + HQQ + the resident matrix | registry, runner, shard/index/skip, atomic writes, quant cache, `--rerun-incomplete`, `--dry-run`, OOM-retry ladder, aggregate, plot, `awq_lite`, `hqq_adapter` with the CPU test, `pile_val` load check, `resident_full.yaml`, smoke config wired into CI, `ptq prefetch`, `ptq cache gc` | `--dry-run` prints a credible estimate; **`resident_full.yaml` completes in one sitting** (models 1-5, expected ~1-2 h) rather than v1's two overnight sessions; a run killed mid-way resumes without re-quantizing; CSV and plots regenerate in one command; AWQ4 g128 below RTN4 g128; opt-1.3b fp16 14.63 and opt-2.7b land on their §13 rows | 2.5 days |
| **M5** Scale up: opt-6.7b streamed | prefetch and run `streamed_opt67b.yaml`; `peak_vram_gb` / `peak_ram_gb` instrumentation proven at scale | **opt-6.7b fp16 wikitext2 10.86 ± 0.03, GPTQ4 per-row 11.39 ± 0.15, RTN4 12.10 ± 0.05**; ptb 15.77 and c4 12.71 advisory; `peak_vram_gb` under 8.0 and `peak_ram_gb` under 23; **opt-1.3b evaluated both resident and streamed agrees to 1e-4**, proving the two paths are one protocol. This is the milestone that constitutes the evidence for server access: a 7B-class model fully quantized and matched to published numbers on an 8 GB laptop GPU | 2 days + one overnight |
| **M6** Llama + v2 backends | `streamed_llama.yaml`, `bnb_adapter` and `torchao_adapter` (neither needs nvcc), optional CUDA 13.0 toolkit → `gptqmodel_adapter` and `verify-backend`, optional lm-eval zero-shot rows, write-up | **Llama-2-7b fp16 5.47 ± 0.02, RTN4 g128 5.72-5.73, GPTQ4 g128 5.61-5.69**, and the other four §13-verified configurations within tolerance; Llama-3.1-8B bf16 completes at least fp/rtn/gptq at 4 bits g128 without OOM (no verified reference, so it is descriptive only); no empty pivot cell in the resident tier; if the toolkit is installed, fake-quant vs gptqmodel within 0.05 | 3-5 days, several overnight |
